...
const Achievements: React.FC = () => {
  ...
  return (
    <div className="col-span-2 rounded-lg bg-white p-4">
      {/* 时长成就 */}
      <div className="mb-4 rounded-lg bg-white p-3 shadow border border-gray-200">
        ...
        <div className="flex items-center justify-between">
          {getCurrentTimeLevel() < TIME_ACHIEVEMENTS.length ? (
            ...
            <span className="text-xs text-gray-500">
              <FaCheck className="inline mr-1" />已完成全部阶段
            </span>
          )}
      ...
      <div className="max-h-[500px] space-y-2 overflow-y-auto">
        {achievementsData
          ...
          .map((achievement) => (
            <div
              key={achievement.id}
              className={`rounded-lg bg-white p-3 shadow border ${achievement.is_completed ? 'border-l-4 border-gray-400' : 'border-gray-200'}`}
            >
              <div className="mb-1 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="rounded bg-gray-100 px-2 py-1 text-xs text-gray-600">{achievement.achievement_type}</span>
                  <span className="font-semibold text-black">{achievement.achievement_name}</span>
                </div>
                <button onClick={() => handleDeleteAchievement(achievement.id)} className="text-gray-400 transition hover:text-gray-600">
                  <FaRegCircleXmark />
                </button>
              </div>
              {achievement.description && <p className="mb-2 text-sm text-gray-700">{achievement.description}</p>}
              <div className="flex items-center justify-between">
                <button
                  onClick={() => handleToggleAchievement(achievement)}
                  className={`rounded px-3 py-1 text-sm text-white transition ${
                    achievement.is_completed ? 'bg-gray-400 hover:bg-gray-500' : 'bg-gray-600 hover:bg-gray-700'
                  }`}
                >
        ...
        {achievementsData?.filter((a) => a.achievement_type !== '时长成就' && a.achievement_type !== '完成度成就').length === 0 && (
          <div className="rounded-lg bg-white p-4 text-center text-gray-400 border border-gray-200">暂无自定义成就,点击上方按钮添加</div>
        )}
      </div>
    </div>
  );
