        ...
        <div className="mb-2 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-black">
              {TIME_ACHIEVEMENTS[Math.max(storedTimeLevel, 1) - 1]?.name ||
                TIME_ACHIEVEMENTS[0].name}
            </span>
          </div>
          <span className="text-xs text-gray-500">
            Lv.{storedTimeLevel}/{TIME_ACHIEVEMENTS.length}
          </span>
        </div>
        <p className="mb-2 text-sm text-gray-700">
          {TIME_ACHIEVEMENTS[Math.max(storedTimeLevel, 1) - 1]?.description ||
            TIME_ACHIEVEMENTS[0].description}
        </p>
        <div className="flex items-center justify-between">
          {nextTimeIdx !== -1 ? (
            <span className="text-xs text-gray-500">
              还需 {TIME_ACHIEVEMENTS[nextTimeIdx].hours - playHours} 小时即可达成下一阶段
            </span>
          ) : (
            <span className="text-xs text-gray-500">
